  ...
  for (int cycle = 0; cycle < duration; cycle++) {
    ...
    for (int y = 0; y < MATRIX_HEIGHT; y++) {
      for (int x = 0; x < MATRIX_WIDTH; x++) {
        ...
        // ESCALADO POR RANGOS (3 secciones)
        int rango = (y * 3) / MATRIX_HEIGHT;

        // Área opcional del águila (centro)
        bool isEagleArea = (x >= MATRIX_WIDTH/2 - 1 && x <= MATRIX_WIDTH/2 &&
                            y >= MATRIX_HEIGHT/2 - 1 && y <= MATRIX_HEIGHT/2);

        if (isEagleArea) {
          strip.setPixelColor(pixelIndex, 0); // negro
        }
        else if (rango == 0) {
          // ROJO (arriba)
...
void loop() {
  ...
  scrollText(reverseString("   UNIT ELECTRONICS"), strip.Color(255, 60, 0), 25);
  // fuegoIndividual(strip.Color(255, 255, 255)); // Fuego blanco
  delay(150);
  

  // 5. Scroll fecha en verde (ACTUALIZADA A 2025)
  scrollText(reverseString(" ECOSISTEMA  DEVLAB "), strip.Color(60, 255, 60), 20);
  // fuegoIndividual(strip.Color(50, 255, 250)); // Fuego verde
  delay(150);
  


  // 8. Scroll "VIVA MEXICO!" en verde
  scrollText(reverseString("   HECHO EN MEXICO!"), strip.Color(100, 205, 220), 25);
